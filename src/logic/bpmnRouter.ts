// bpmnRouter.ts - VERSION AVEC SUPPORT HAUTEURS VARIABLES (LOGIQUE TRAJECTOIRES PRÉSERVÉE)
...
/**
 * 🎯 ROUTER INTELLIGENT - Logique préservée + support hauteurs variables
 */
export class BPMNRouter {
    ...
    /**
     * OBTENIR POINT DE CONNEXION - AVEC SUPPORT HAUTEURS VARIABLES
     */
    private getConnectionPoint(
        ...
        // Récupérer la hauteur réelle du node
        const nodeHeight = this.getNodeHeight(row);
        ...
        if (row.typeBpmn === BPMN_TYPES.START_EVENT || row.typeBpmn === BPMN_TYPES.END_EVENT) {
            ...
            // Task standard - UTILISER LA HAUTEUR VARIABLE
    ...
    /**
     * OBTENIR LA HAUTEUR D'UN NODE
     */
    private getNodeHeight(row: Table1Row): number {
        // Si on a une hauteur stockée, l'utiliser
        if (this.nodeHeights.has(row.id)) {
            return this.nodeHeights.get(row.id)!;
        }

        // Sinon, valeurs par défaut selon le type
        if (row.typeBpmn === BPMN_TYPES.START_EVENT || row.typeBpmn === BPMN_TYPES.END_EVENT) {
            return this.config.eventSize;
        }
        if (row.typeBpmn === BPMN_TYPES.EXCLUSIVE_GATEWAY) {
    ...
    /**
     * OBTENIR Y MAX GLOBAL (pour contournements)
     */
    private getGlobalMaxY(): number {
        let maxY = 0;

        this.arrows.forEach(arrow => {
            const sourceHeight = this.getNodeHeight(arrow.sourceRow);
            const targetHeight = this.getNodeHeight(arrow.targetRow);

            const sourceEndY = arrow.sourcePos.y + sourceHeight;
            const targetEndY = arrow.targetPos.y + targetHeight;

            if (sourceEndY > maxY) maxY = sourceEndY;
            if (targetEndY > maxY) maxY = targetEndY;
        });
